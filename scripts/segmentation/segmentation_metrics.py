...
def combined_ce_dice_loss(
    ...
    *,
    ...
    exclude_background_from_dice: bool = True,
...
def metrics_from_confusion(
    ...
    *,
    ...
    valid = gt > 0
    if exclude_background_from_macro and len(valid) > 1:
        valid[0] = False

    return {
        "macro_dice": float(torch.nanmean(dice[valid])) if torch.any(valid) else float("nan"),
        "macro_iou": float(torch.nanmean(iou[valid])) if torch.any(valid) else float("nan"),
